...
"""
Datawrapper for pi0 finetune
"""

class Pi0UR5DataWrapper(Wrapper):
    def __init__(self, env, directory, prompt,collect_freq=1, flush_freq=100):
        """
        Initializes the data collection wrapper.
        ...
        Args:
            ...
            collect_freq (int): How often to save simulation state, in terms of environment steps.
            flush_freq (int): How frequently to dump data to disk, in terms of environment steps.
        Wrapper for collecting UR5 trajectories in robosuite, ready for OpenPI π₀ fine-tuning.
        Each episode is dumped为:
        ep_xxxx/
            ├─ states.npy          # (T,7)  float32
            ├─ action_infos.npy    # (T,7)  float32  (Δ joint + gripper)
            ├─ base_rgb.npy        # uint8, (T,256,256,3)
            ├─ wrist_rgb.npy       # uint8, (T,256,256,3)
            ├─ meta.json           # prompt / task / success 等
            └─ model.xml           # 复现用
        """
        super().__init__(env)
        self.prompt=prompt
        ...
        self.states = []
        self.action_infos = []
        self.base_imgs, self.wrist_imgs = [], []
        self.successful = False  # stores success state of demonstration
    ...
    def _flush(self):
        ...
        np.savez(
            ...
            states=np.array(self.states),
            action_infos=self.action_infos,
            base_rgb=self.base_imgs,
            wrist_rgb=self.wrist_imgs,
            successful=self.successful,
            env=env_name,
        )
        self.states = []
    ...
    def step(self, action):
        ...
        if self.t % self.collect_freq == 0:
            ...
            self.action_infos.append(np.array(action))
            base = ret[0]["frontview_image"][::-1,::-1]
            wrist =ret[0]["robot0_eye_in_hand_image"][::-1,::-1]
            self.base_imgs.append(base.astype(np.uint8))
            self.wrist_imgs.append(wrist.astype(np.uint8))
        # check if the demonstration is successful
        if self.env._check_success():
            self.successful = True
